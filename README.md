...
# Release notes
## 4.124
-add timers recording
Запись доступна тдля 'ID плеера' 1 или 4097(только при  использовании Gstreamer),почти все на HiSilicon отпадает.Octagon (Sf8008 и др.)точно использует  Gstreamer.
Необходимо заново выбрать скин и раскладку кнопок в общем меню настроек,там же  есть настройка "Дефолтное время старта записей для архива".
То есть,если вы выбрали запись архива(окно с  EPG/окно с  деталями EPG,дальше кнопка меню),то старт записи архива начнется в дефолтное время на следующий день.Что бы не накладывались двойные запросы на сервер при текущем просмотре ТВ.Если не важно,то поменяйте время вручную сами, при установке таймера.
Мгновенная запись доступна при просмотре ТВ,кнопка запись на пульте(кнопка доступна если не смотрите архив).
Иконка в инфобаре появится только тогда,когда идет запись с текущего сервера.Если запись со спутника и т.д.,то иконки не будет.

# Release notes
## 4.123
-fix updateSource(openBH/openViX)
File "/usr/lib/enigma2/python/Navigation.py", line 216, in playService
AttributeError: 'Event' object has no attribute 'updateSource'
...
-лево/право инфобар
- нажать ок во время паузы плей
- нейтрино --> лево/право инфобар, кнопка видео PiP

# Release notes
## 4.118

- исправлено скинуть пароль в настройках
- пауза/плей по зеленой кнопке

# Release notes
## 4.117

- добавлено в настройки плейлистов "Примеры EPG" (по желтой кнопке)

# Release notes
## 4.116

- добавлен таймер переключения:
1) список каналов - кнопка меню - таймер переключения;
2) открыть ЕПГ канала - курсор на будущем событии кнопка меню (первый раз необходимо выбрать еще раз раскладку кнопок в настройках). 

# Release notes
## 4.115

-добавлено меню PiP по кнопке video/movie(скорей всего будет работать только openPLi)
-попытался вывести на дисплей кое какую информацию,в случае архива время/прогресс бар будут неверные
...
- open providers if not answer(EPG htpp://technic.cf/)
...
- желтая кнопка - новый список истории каналов

# Release notes
## 4.110

- добавлен вывод номера в списке каналов, альтернативная нумерация каналов
...
- Переделана логика cid hash с url плейлиста возвращает каждый раз другой(уникальный) номер,фавориты из-за этого не сохраняются,
в этом случае будет использовать временно num

## 4.109

- Исправлен tvteam py3 и д.р.

## 4.108

- обновлен url cbilling

## 4.107

- Исправлен формат епг для шуры
...
## 4.104
- Поддержка прошивок с древней версией twisted<14

## 4.103

- Исправлено EPG на итв-лайв

## 4.102

- Исправлен крэш при перемещении избранных каналов

## 4.101

- Обновление антифриз в связи с изменениями на стороне провайдера

## 4.100

- Исправлен крэш при сортировке

## 4.99

- Исправлено отображение названия передачи - больше не обрезается
- Стопкадр при включении архива на паузу
- Обновляются индикаторы прогресса передачи в списке каналов
- Файлы *.py добавлены в пакет для последующей совместимости с питон 3
- Перевод заголовков
